...
const accelerate = ({ style }, keyframes) =>
  (style.willChange = keyframes ? keyframes.map(({ property }) => property).join() : 'auto');
...
const easings = {
  ...
  'in-out-cubic': progress =>
    (progress *= 2) < 1 ? 0.5 * progress ** 3 : 0.5 * ((progress -= 2) * progress ** 2 + 2),
  'in-out-quartic': progress =>
    (progress *= 2) < 1 ? 0.5 * progress ** 4 : -0.5 * ((progress -= 2) * progress ** 3 - 2),
  'in-out-quintic': progress =>
    (progress *= 2) < 1 ? 0.5 * progress ** 5 : 0.5 * ((progress -= 2) * progress ** 4 + 2),
  'in-out-exponential': progress =>
    (progress *= 2) < 1 ? 0.5 * 1024 ** (progress - 1) : 0.5 * (-(2 ** (-10 * (progress - 1))) + 2),
  ...
  'in-out-elastic': (progress, amplitude, period) => {
    ...
    return (progress *= 2) < 1
      ? -0.5 *
          (strength * 2 ** (10 * (progress -= 1)) * Math.sin((progress - offset) * pi2 / period))
      : strength *
          2 ** (-10 * (progress -= 1)) *
